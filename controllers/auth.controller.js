...
exports.updateCart = async (req,res,next)=>{
    ...
    try {
        ...
        const books = await Book.find({$in:sessionItems.map(item=>item.id)}).select("_id quantity");
...
sendMail = (mailOptions,res) =>{
    ...
    transporter.sendMail(mailOptions, function (err,info) {
        ...
        res.status(200).send({message: 'Succesful. A verification email has been sent to your email.'});
    });
}